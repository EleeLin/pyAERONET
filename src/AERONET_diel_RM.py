...
# regions
regions = ['West_N', 'West_S', 'Central_N', 'Central_S', 'East_N', 'East_S']
regions_pos = [[-125.0, -104.7, 37.0, 49.0], [-125.0, -104.7, 25.0, 37.0], [-104.7, -85.3, 37.0, 49.0], [-104.7, -85.3, 25.0, 37.0], [-85.3, -66.0, 37.0, 49.0], [-85.3, -66.0, 25.0, 37.0]]
...
for rr in range (0, len (regions)):
    for oo in range (0, len (mon_str)):
        gc_diel_data = np.zeros (24)
        aeronet_diel_data = np.zeros (24)
        diel_ct = np.zeros (24)
        ...
        for ii in range (0, 24):
            for jj in range (0, 366):
                ...
                for kk in range (0, 1303):
                    ...
                    if (lon > regions_pos [rr] [1] or lon < regions_pos [rr] [0] or lat > regions_pos [rr] [3] or lat < regions_pos [rr] [2]):
                        continue 
                    ...
                    if (math.isnan (aeronet_aod_value) == False and math.isnan (gc_aod_value) == False):
                        if (aeronet_aod_value > 0.0 and gc_aod_value > 0.0):
                            aeronet_diel_data [ii] += aeronet_aod_value
                            gc_diel_data [ii] += gc_aod_value 
                            diel_ct [ii] += 1.0

        for i in range (0, 24):
            if diel_ct [i] > 0.0:
                aeronet_diel_data [i] /= diel_ct [i]
                gc_diel_data [i] /= diel_ct [i]

        outfile = open ('./diel_' + regions [rr] + '_' + mon_str [oo] + '.csv', 'w')
        with outfile:
            writer = csv.writer (outfile)
            writer.writerow (aeronet_diel_data)
            writer.writerow (gc_diel_data)
            writer.writerow (diel_ct)
