...
boundary = [-125.0, -66.0, 25.0, 49.0]

outfile = open ('./AERONET_sites_US.csv', 'w')
with outfile:
    writer = csv.writer (outfile)
    for i in range (0, len (AERONET_data [0])):
        if AERONET_data [1] [i] > boundary [1] or AERONET_data [1] [i] < boundary [0] or AERONET_data [0] [i] > boundary [3] or AERONET_data [0] [i] < boundary [2]:
            continue

        writer.writerow ([AERONET_data [1] [i], AERONET_data [0] [i]])
